...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count) {
    ...
    *result_count = player_count;
    ...
    // Step 1: build unique scores in descending order
    int* unique = malloc(ranked_count * sizeof(int));
    int unique_count = 0;

    unique[unique_count++] = ranked[0];  // first always included
    ...
    // Step 2: traverse player's scores (ascending order given in input)
    int index = unique_count - 1; // start from lowest unique score
    for (int i = 0; i < player_count; i++) {
        int score = player[i];

        // Move upward while player beats current leaderboard scores
        while (index >= 0 && score >= unique[index]) {
            index--;
        }

        // rank = index+2 (since index now points just above player's position)
        result[i] = index + 2;
    }
