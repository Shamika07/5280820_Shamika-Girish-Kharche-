...
int hackerlandRadioTransmitters(int x_count, int* x, int k) {
    ...
    while (i < x_count) {
        ...
        // Step 1: find farthest house within range k from current house
        int loc = x[i] + k;
        while (i < x_count && x[i] <= loc) {
            i++;
        }

        // Step 2: place transmitter at the farthest house within range
        int transmitterPos = x[i - 1];

        // Step 3: skip houses covered by transmitter
