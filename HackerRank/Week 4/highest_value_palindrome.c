...
char *result = strdup(s);  // copy of original string
    int *changed = calloc(n, sizeof(int)); // track modified positions
    int used = 0;

    // Step 1: Make palindrome minimally
    for (int i = 0; i < n / 2; i++) {
        if (result[i] != result[n - 1 - i]) {
            char maxc = (result[i] > result[n - 1 - i]) ? result[i] : result[n - 1 - i];
            result[i] = result[n - 1 - i] = maxc;
            changed[i] = changed[n - 1 - i] = 1;
            used++;
        }
    }

    if (used > k) { // not possible
        free(changed);
        return strdup("-1");
    }

    // Step 2: Maximize palindrome (make '9's where possible)
    ...
    // If odd length, middle can be upgraded
