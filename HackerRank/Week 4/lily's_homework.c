...
// Function to count swaps to turn arr[] into target[]
int countSwaps(int n, int* arr, int* target) {
    int* visited = calloc(n, sizeof(int));
    int* indexMap = malloc(n * sizeof(int));
    int swaps = 0;

    // map values -> target index
    for (int i = 0; i < n; i++) {
        indexMap[target[i]] = i;
    }
...
int lilysHomework(int arr_count, int* arr) {
    ...
    int ascSwaps = countSwaps(arr_count, arr, asc);
    int descSwaps = countSwaps(arr_count, arr, desc);
